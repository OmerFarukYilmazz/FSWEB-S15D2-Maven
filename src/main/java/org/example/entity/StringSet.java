...
public class StringSet {
    public static Set<String> findUniqueWords(){
        ...
        String[] cleanedText = text.toLowerCase().replaceAll("[^a-zA-Z\\s]", "").split("\\s+");
        System.out.println("before process " + cleanedText.length);

        TreeSet<String> uniqueWords = new TreeSet<>(Arrays.asList(cleanedText));
